...
function setupEventListeners() {
    ...
    if (postForm) {
        postForm.removeEventListener('submit', handlePostSubmit); // 移除可能存在的监听器
    ...
    if (themeForm) {
        themeForm.removeEventListener('submit', handleThemeSubmit);
    ...
    if (githubForm) {
        githubForm.removeEventListener('submit', handleGitHubSubmit);
    ...
    if (postContent) {
        postContent.removeEventListener('input', updateMarkdownPreview);
    ...
    if (postTitle) {
        postTitle.removeEventListener('input', generateSlug);
...
async function handleChangePassword(e) {
    ...
    if (newPassword.length < 8) {
        showNotification('新密码至少需要8位字符', 'error');
        return;
    }
    ...
    try {
        // 创建认证实例
        const auth = new BlogAuth();
        const settings = auth.getAuthSettings();

        // 验证当前密码
        const currentPasswordHash = await auth.hashPassword(currentPassword, settings.salt);
        if (currentPasswordHash !== settings.passwordHash) {
        ...
        const newPasswordHash = await auth.hashPassword(newPassword, newSalt);

        // 更新设置
        settings.salt = newSalt;
        settings.passwordHash = newPasswordHash;
        auth.saveAuthSettings(settings);
        ...
        showNotification('密码更改失败：' + error.message, 'error');
    }
}

function clearAllSessions() {
    if (confirm('确定要清除所有登录会话吗？这将强制所有设备重新登录。')) {
        // 清除当前会话
        localStorage.removeItem('blog_session');
        showNotification('所有会话已清除，即将跳转到登录页面', 'success');
...
function resetAllData() {
    ...
    if (confirmed) {
        const doubleConfirm = confirm('请再次确认：您真的要删除所有数据吗？');

        if (doubleConfirm) {
            // 清除所有本地存储数据
            localStorage.clear();

            showNotification('所有数据已清除，即将跳转到登录页面', 'success');
